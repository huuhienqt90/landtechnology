...
class ResultPrinter
{
    ...
    {
        $data = null;
        if ($exception instanceof \Hamilton\PayPal\Exception\PayPalConnectionException) {
    ...
    protected static function printConsoleObject($object, $error = null)
    {
        ...
        if ($object) {
            if (is_a($object, 'PayPal\Common\PayPalModel')) {
                /** @var $object \Hamilton\PayPal\Common\PayPalModel */
                echo $object->toJSON(128);
            } elseif (is_string($object) && \Hamilton\PayPal\Validation\JsonValidator::validate($object, true)) {
